<template>
  <div class="recharge-lists" v-if="statu == '400'">
    <div class="recharge-list pre-price" v-for="item in showPrice">{{item}}元</div>
  </div>
  <div class="recharge-lists" v-else-if="statu == '200'">
    <div class="recharge-list disc-price" v-for="(item,index) in price"  @click="selectItem(item.showP,item.realP)">
      {{item.showP}}元
      <span>售价:{{item.realP}}元</span>
    </div>
  </div>
</template>
...
<script>
  export default {
    ...
    created () {
    	this.$http.get("/api/telFare").then((res) => {
